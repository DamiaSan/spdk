...
function raid_delta_map() {
	local raid_level=$1
	local num_base_bdevs=$2
	local background_io=$3
	local base_bdevs=($(for ((i = 1; i <= num_base_bdevs; i++)); do echo BaseBdev$i; done))
	local raid_bdev_name="raid_bdev1"
	local strip_size=0
	local create_arg
	...
	if [ $raid_level != "raid1" ]; then
		echo "skipping delta map test for level $raid_level"
		return 1
	fi

	"$rootdir/build/examples/bdevperf" -r $rpc_server -T $raid_bdev_name -t 60 -w randrw -M 20 -o 4M -q 2 -U -z -L bdev_raid &
	raid_pid=$!
	waitforlisten $raid_pid $rpc_server

	# Create base bdevs
	for bdev in "${base_bdevs[@]}"; do
		$rpc_py bdev_malloc_create 32 512 -b $bdev -o 8
	done

	# Create RAID bdev
	$rpc_py bdev_raid_create -r $raid_level -b "${base_bdevs[*]}" -n $raid_bdev_name
	echo $raid_bdev_name $raid_level $strip_size $num_base_bdevs
	verify_raid_bdev_state $raid_bdev_name "online" $raid_level $strip_size $num_base_bdevs
