...
struct rpc_bdev_lvol_set_local_parent {
	char *lvol_name;
	char *snapshot_name;
};

static void
free_rpc_bdev_lvol_set_local_parent(struct rpc_bdev_lvol_set_local_parent *req)
{
	free(req->lvol_name);
	free(req->snapshot_name);
}

static const struct spdk_json_object_decoder rpc_bdev_lvol_set_local_parent_decoders[] = {
	{"lvol_name", offsetof(struct rpc_bdev_lvol_set_local_parent, lvol_name), spdk_json_decode_string},
	{"snapshot_name", offsetof(struct rpc_bdev_lvol_set_local_parent, snapshot_name), spdk_json_decode_string},
};

static void
rpc_bdev_lvol_set_local_parent_cb(void *cb_arg, int lvolerrno)
{
	struct spdk_jsonrpc_request *request = cb_arg;
...
static void
rpc_bdev_lvol_set_local_parent(struct spdk_jsonrpc_request *request,
			       const struct spdk_json_val *params)
{
	struct rpc_bdev_lvol_set_local_parent req = {};
	struct spdk_lvol *lvol, *snapshot;
	struct spdk_bdev *lvol_bdev, *snapshot_bdev;

	SPDK_INFOLOG(lvol_rpc, "Set parent of lvol\n");

	if (spdk_json_decode_object(params, rpc_bdev_lvol_set_local_parent_decoders,
				    SPDK_COUNTOF(rpc_bdev_lvol_set_local_parent_decoders),
				    &req)) {
		SPDK_INFOLOG(lvol_rpc, "spdk_json_decode_object failed\n");
		spdk_jsonrpc_send_error_response(request, SPDK_JSONRPC_ERROR_INTERNAL_ERROR,
						 "spdk_json_decode_object failed");
	...
	spdk_lvol_set_local_parent(lvol, snapshot, rpc_bdev_lvol_set_local_parent_cb, request);

cleanup:
	free_rpc_bdev_lvol_set_local_parent(&req);
}

SPDK_RPC_REGISTER("bdev_lvol_set_local_parent", rpc_bdev_lvol_set_local_parent, SPDK_RPC_RUNTIME)
