...
void
spdk_lvol_set_local_parent(struct spdk_lvol *lvol, struct spdk_lvol *snapshot,
			   spdk_lvol_op_complete cb_fn, void *cb_arg)
{
	struct spdk_lvol_req *req;
	...
	if (!spdk_blob_is_esnap_clone(lvol->blob)) {
		SPDK_ERRLOG("%s is not an esnap clone\n", lvol->name);
		cb_fn(cb_arg, -EPERM);
		return;
	}
	...
	spdk_bs_blob_set_local_parent(lvol->lvol_store->blobstore, blob_id, snapshot_id,
